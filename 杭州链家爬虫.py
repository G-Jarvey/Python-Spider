...
# url示例：https://hz.lianjia.com/ershoufang/xihu/pg2/
...
for key_, value_ in area_dic.items():
    ...
    for i in tqdm(range(total_page), desc=key_):
        ...
        for info in info_collect:
            ...
            #房间类型
            info_dic['hourseRoom'] = icons[0].strip()
            #房间面积
            ...
            #匹配关注信息
            #关注人数
            followInfo = re.findall('<span class="starIcon"></span>(.*?)</div>',str(info))[0].strip().split('/')
            info_dic['followers'] = followInfo[0].strip()
